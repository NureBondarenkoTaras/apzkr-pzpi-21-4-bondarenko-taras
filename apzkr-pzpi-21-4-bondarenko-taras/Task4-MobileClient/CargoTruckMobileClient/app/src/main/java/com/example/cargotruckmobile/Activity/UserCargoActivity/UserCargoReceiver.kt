...
import com.example.cargotruckmobile.Activity.Adapter.CargoAdapter
...
class UserCargoReceiver : AppCompatActivity() {
    ...
    private fun fetchCargoData(id: String) {
        lifecycleScope.launch(Dispatchers.Main) {
            try {
                val cargoList = RetrofitClient.cargoService.getCargoByReceiver(id)
                cargos.clear()
                cargos.addAll(cargoList)
                adapter.notifyDataSetChanged()
                Log.d("Cargo", "Data fetched successfully: ${cargoList.size} items")
            } catch (e: Exception) {
